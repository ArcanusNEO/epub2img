...
void read_from_opf(const char* __path, const char* __out) {
    ...
    for (i = -1, ch = fgetc(fp); ~ch; ch = fgetc(fp)) {
        ...
        if (ch == _CONTAINERSTR[++i]) {
            if (i == _CONTAINERN - 2) {
                for (buffer[0] = '/', i = 1, ch = fgetc(fp); ~ch && ch != '\"'; ch = fgetc(fp), ++i) {
                    buffer[i] = ch;
                }
                buffer[i] = '\0';
                break;
    ...
    for (j = strlen(path), i = j - 1; path[i] != '/'; --i) {
        path[i] = '\0';
    }
    for (i = -1, j = -1, k = -1, ch = fgetc(fp); ~ch; ch = fgetc(fp)) {
        ...
        if (ch == _TYPESTR[++k]) {
            if (k == _TYPEN - 2) {
                for (i = 0, ch = fgetc(fp); (i <= _JPEGN - 2 && ch == _JPEGSTR[i]) || (i <= _PNGN - 2 && ch == _PNGSTR[i]); ch = fgetc(fp), ++i) {
                    NULL;
                }
                if (ch == '\"' && (i == _JPEGN - 1 || i == _PNGN - 1)) {
                    strcpy(cmd, "mv \"");
                    for (i = 4, j = i; path[i - j]; ++i) {
                        cmd[i] = path[i - j];
                    }
                    for (j = i; buffer[i - j]; ++i) {
                        cmd[i] = buffer[i - j];
                    }
                    cmd[i++] = '\"';
                    cmd[i++] = ' ';
        ...
        for (i = -1, ch = fgetc(fp); ~ch; ch = fgetc(fp)) {
            ...
            if (ch == _IMGSTR[++i]) {
                if (i == _IMGN - 2) {
                    ...
                    for (k = _MAX_FILE_NAME - 2, ext[_MAX_FILE_NAME - 1] = '\0'; cmd[i - _MAX_FILE_NAME + 1 + k] != '.'; --k) {
